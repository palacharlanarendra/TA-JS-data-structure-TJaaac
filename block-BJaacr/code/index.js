...
let housesLength = got.houses;
console.log(Object.keys(housesLength).length);
...
let sum=``;
for(let i=0;i<got.houses.length;i++){
  if(i!=got.houses.length-1){
    sum+=`${got.houses[i].name}: ${got.houses[i].people.length}, `;
  }else{
    sum+=`${got.houses[i].name}: ${got.houses[i].people.length}`;
  }
}
...
console.log(allPeople +"and its length is:"+allPeople.length);
...
console.log(allDescription +"and its length is:"+allDescription.length);
